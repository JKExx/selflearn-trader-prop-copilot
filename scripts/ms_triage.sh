...
ms_num() {
  t="$1"
  num="$(gh api repos/$OWNER/$REPO/milestones -f state=open   --jq ".[] | select(.title==\"$t\") | .number" | head -n1 || true)"
  if [ -z "$num" ]; then
    num="$(gh api repos/$OWNER/$REPO/milestones -f state=closed --jq ".[] | select(.title==\"$t\") | .number" | head -n1 || true)"
  fi
  echo "$num"
}
...
ensure_ms() {
  ...
  n="$(ms_num "$t")"
  if [ -z "$n" ]; then
    n="$(gh api -X POST repos/$OWNER/$REPO/milestones -f title="$t" -f state='open' --jq .number)"
  fi
  echo "$n"
}
...
case "$M1" in (*[!0-9]*|"") echo "Bad milestone id for v0.3.1: $M1"; exit 1;; esac
case "$M2" in (*[!0-9]*|"") echo "Bad milestone id for v0.3.2: $M2"; exit 1;; esac
