...
ms_num() {
  ...
  if [ -z "$num" ]; then
    num=$(gh api -X POST repos/$OWNER/$REPO/milestones -f title="$title" -f state='open' --jq .number)
  fi
  echo "$num"
}
...
case "$M1" in (*[!0-9]*|"") echo "Bad milestone for v0.3.1: $M1"; exit 1;; esac
case "$M2" in (*[!0-9]*|"") echo "Bad milestone for v0.3.2: $M2"; exit 1;; esac
